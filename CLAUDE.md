...
5. **[FAIT — constate le 09/08/2026, complete le 17/08/2026]** Encart/lien test-eligibilite deja present dans les templates prix ville (600 pages, + lien vers page prix nationale) et dept (306 pages). Le lien vers la page prix NATIONALE dans PrixDeptContent.astro (action 1.3 du plan croissance) est DEJA en place (`cfg.guide`, ligne ~141 « Pour aller plus loin ») — constate le 17/08, rien a ajouter.
...
### Decisions Robin du 24/08/2026 — 2e serie « opportunites » (go 1/2/4/5/6, prix garde)

1. **Sync GSC corrige** (scripts/sync-analytics.mjs) : pagination jusqu'a 75k lignes + conservation de TOUTE ligne avec >= 1 clic (l'ancien filtre top-500 pages jetait la longue traine — cause de la keep-list aveugle). Routine : verifier le volume de lignes au prochain sync (attendu : nettement > 25k brutes).
2. **Cluster expat/EN lance** : page pilier `/en/glp1-in-france/` (prix regules vs USA, prescriptions, remboursement). Suite = axe E du content-plan. Le Coach repond en anglais (v78). Dossier EN : a terme, pas prioritaire.
3. **PRIX DOSSIER : RESTE A 4,99 EUR** (decision explicite Robin 24/08 « garde 4,99 ») — ne PAS re-proposer de changement de prix.
4. **NEWSLETTER HEBDO « Actu GLP-1 » (OBLIGATION ROUTINE, chaque lundi)** : rediger et envoyer via send-feedback-email (category `newsletter_weekly`) aux contacts opt-in (newsletter=true, moins STOP/incoming repliers, adresses gmail normalisees, pied de page « repondez STOP »). Contenu : 3-4 actus sourcees (pipeline axe A : orforglipron/CagriSema/retatrutide, remboursement, nouveaute site) + 1 CTA funnel (test ou Dossier). Premiere edition : lundi 31/08/2026. Compteur envois/STOP dans le rapport A0.
5. **Stock communautaire MVP deploye** : colonne `availability` dans pharmacy_prices (disponible/rupture/sur_commande, prix devenu optionnel — contrainte l'un-ou-l'autre), formulaire de la carte reoriente « signaler la disponibilite ». Routine : compter les signalements chaque run ; a >= 20 signalements, proposer l'affichage (badges dispo carte + pages pharmacies).
6. **Instrumentation K2** : events GA4 `dossier_form_step` (par etape) + `dossier_form_submit` sur /dossier-glp1/. Routine : analyser la fuite etape par etape des ~50 events. Hotjar : demander UNE fois a Robin de visionner ~10 enregistrements de la landing (action manuelle cote lui).
...
3. **Cannibalisation** [EN COURS — 5/42 au 17/08] : 42 requetes >= 100 imp/28j servies par 3+ pages. Traiter 5/jour : choisir la canonique, desoptimiser les titles concurrents, maillage convergent. Compteur x/42 dans le rapport. Lot 1 (17/08) : (1) « carte prix mounjaro france » → canonique = carte (pos 3,9, CTR 19 %), keyword retire de prix-mounjaro-france ; (2) « mounjaro pharmacie moins cher autour de moi » → canonique = carte, renvoi deja en place dans prix-mounjaro (constate), keyword retire ; (3) « mounjaro prix le moins cher » → pas de cannibalisation reelle (prix-mounjaro domine, probleme = position domaine) ; (4) « ozempic prix » → concurrents = pages dept/ville pos 28-77, bruit, lien national deja en place ; (5) « prix ozempic sans ordonnance » → canonique prix = prix-ozempic-france (deja lie a l'article dedie) + VRAI fix : les 2 articles sans-ordonnance se cannibalisaient entre eux (titles quasi identiques) → legal-risques repositionne sur « faux ozempic / contrefacons / recours » avec lien canonique vers achat-en-ligne-france.
